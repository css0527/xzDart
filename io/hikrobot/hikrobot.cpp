...
void HikRobot::capture_start()
{
  ...
  capture_thread_ = std::thread{[this] {
    ...
    while (!capture_quit_) {
      ...
      cvt_param.pDstBuffer = nullptr;  // Will set later
      cvt_param.nDstBufferSize = 0;
      ...
      if (pixel_type == PixelType_Gvsp_BGR8_Packed) {
        img = cv::Mat(cv::Size(frame_info.nWidth, frame_info.nHeight), CV_8UC3, raw.pBufAddr);
      } else if (pixel_type == PixelType_Gvsp_RGB8_Packed) {
        img = cv::Mat(cv::Size(frame_info.nWidth, frame_info.nHeight), CV_8UC3, raw.pBufAddr);
        cv::cvtColor(img, img, cv::COLOR_RGB2BGR);
      } else {
        // Assume Bayer
        img = cv::Mat(cv::Size(frame_info.nWidth, frame_info.nHeight), CV_8U, raw.pBufAddr);
        cv::Mat dst_image;
        ...
        if (it != type_map.end()) {
          ...
          tools::logger()->warn("Unknown pixel type: {}, assuming BGR", static_cast<int>(pixel_type));
          img = cv::Mat(cv::Size(frame_info.nWidth, frame_info.nHeight), CV_8UC3, raw.pBufAddr);
        }
      }
